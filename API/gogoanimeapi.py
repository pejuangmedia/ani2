...
class gogoanime():
    ...
    def get_search_results(query):
        try:
            url1 = f"https://gogoanime.sk/search.html?keyword={query}"
            session = HTMLSession()
    ...
    def get_anime_details(animeid):
        try:
            animelink = 'https://gogoanime.sk/category/{}'.format(animeid)
...
#                 genres.append(link.get('title'))
    ...
    def get_episodes_link(animeid, episode_num):
        try:
            animelink = f'https://gogoanime.sk/category/{animeid}'
            response = requests.get(animelink)
            plainText = response.text
            soup = BeautifulSoup(plainText, "lxml")
            lnk = soup.find(id="episode_page")
            ...
            tit_url = soup.find("div", {"class": "anime_info_body_bg"}).h1.string
            URL_PATTERN = 'https://gogoanime.fi/{}-episode-{}'
    ...
    def jugad(animeid, episode_num):
        url = f"https://gogoanime.sk/{animeid}-episode-{episode_num}"
